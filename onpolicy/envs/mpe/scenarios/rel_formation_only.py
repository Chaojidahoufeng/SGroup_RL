...
class Scenario(BaseScenario):
    def make_world(self, arglist):
        ...
        world.dim_c = 0
        #world.damping = 1
        world.num_agents = self.args.num_agents
        ...
        for i in range(world.num_agents):
            world.ideal_topo_point[0].append(world.ideal_side_len / np.sqrt(2) * np.cos(i/world.num_agents*2*np.pi))
            world.ideal_topo_point[1].append(world.ideal_side_len / np.sqrt(2) * np.sin(i/world.num_agents*2*np.pi))
    ...
    def reward(self, agent, world):
        ...
        main_reward = self.reward(agent, world)
    ...
    def observation(self, agent, world):
        ...
        collide = []
        obs = world.agents + world.static_obs
        for entity in obs:
            if entity == agent:
                continue
